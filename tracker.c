...
void LoadConfigFile(struct TConfig *Config)
{
	...
	ReadBoolean(fp, "enable_veml6070", -1, 0, &(Config->EnableVEML6070));
	if (Config->EnableVEML6070)
	{
		printf("VEML6070 Enabled\n");
	}
...
int main(void)
{
	...
	struct stat st = {0};
	struct TGPS GPS;
	pthread_t PredictionThread, LoRaThread, APRSThread, GPSThread, DS18B20Thread, ADCThread, CameraThread, BMP085Thread, BME280Thread, LEDThread, LogThread, PipeThread, VEML6070Thread;
	...
	*Config.Channels[1].SSDVFolder = '\0';										// No folder for APRS images
	...
	if (Config.EnableVEML6070)
	{
		...
		{
			fprintf(stderr, "Error creating VEML6070 thread\n");
			return 1;
		}
	}
